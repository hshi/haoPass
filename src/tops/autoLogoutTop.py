...
#==============================
#Top level Class: AutoLogoutTop
#==============================
class AutoLogoutTop(tk.Toplevel):
  ...
  def __create_widgets(self):
    ...
    #Set var to auto_logout
    default_value = "{:.1f}".format(self.container.timeout_min)
    self.var.set(default_value) 

    #Create label and radiobuttons
    oneLabel = tk.Label(self, text="The database will be be disconnected in:")
    oneLabel.grid(row = 0, column = 0, padx=10, pady=(10,5), sticky='NSW') 

    radio1 = tk.Radiobutton(self, text="2 minutes", variable=self.var, value="2.0", command=self.selection)
    radio1.grid(row = 1, column = 0, padx=50, pady=5, sticky='NSW') 
    
    radio2 = tk.Radiobutton(self, text="5 minutes", variable=self.var, value="5.0", command=self.selection)
    ...
    radio3 = tk.Radiobutton(self, text="30 minutes", variable=self.var, value="30.0", command=self.selection)
    radio3.grid(row = 4, column = 0, padx=50, pady=5, sticky='NSW') 

    radio3 = tk.Radiobutton(self, text="1 hour", variable=self.var, value="60.0", command=self.selection)
  ...
  def selection(self):
    ...
    self.container.timeout_min = v
    self.container.reset_timer()